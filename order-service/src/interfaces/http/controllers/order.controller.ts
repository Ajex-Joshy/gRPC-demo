import type { CreateOrder } from "@application/use-cases/create-order.use-case";
import type { GetOrderById } from "@application/use-cases/get-order-by-id.use-case";
import type { GetOrderByUser } from "@application/use-cases/get-order-by-user.use-case";
import type { UpdateOrderStatus } from "@application/use-cases/update-order-status.use-case";
...
export class OrderController {
	...
	updateStatus = async (req: AuthRequest, res: Response) => {
		...
		const order = await this.updateOrderStatusUC.execute(
			id as string,
			status,
			adminId,
		);
